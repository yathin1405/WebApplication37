...
using static WebApplication37.Models.Tour;

namespace WebApplication37.Controllers
{
    public class UserToursController : Controller
    {
        private WebApplication37Context db = new WebApplication37Context();
        ...
        public ActionResult Create([Bind(Include = "GuestID,Num_Adults,Num_Kids,Email,Tour_Name,Tour_Type,Deposit,Discount,Total_Tickets,Total_Cost,GuestCost,Tour_Cost")] UserTour userTour)
        {
            if (ModelState.IsValid)
            {
                //var myskill = new List<ConvertEnum>();
                //foreach (Tour.Tours lang in Enum.GetValues(typeof(Tour.Tours)))
                //    myskill.Add(new ConvertEnum
                //    {
                //        Value = (int)lang,
                //        Text = lang.ToString()
                //    });
                //ViewBag.TourType = myskill;
                //Tour.Tours = userTour.Tour_Name();
